...
<section id="page">
    ...
    <a href="?command=deposit&start=${pageStart - perPage}"><<</a>
    ...
    <header>
        ...
        <nav class="bottom">
            ...
            <ul>
                <li><a href="?command=deposit&start=${pageStart}&language=en"><fmt:message
                        key="deposit.language.en"/></a></li>
                <li><a href="?command=deposit&start=${pageStart}&language=ru" }><fmt:message
        ...
        <nav class="top">
            <ul>
                <c:forEach var="games" items="${result}" varStatus="loop" begin="${pageStart}"
                    ...
                    <li><a href="#article${loop.index+1}">${games.key.getName()}</a></li>
                </c:forEach>
            </ul>
        </nav>
    ...
    <section id="articles">
        <c:forEach var="games" items="${result}" varStatus="loop" begin="${pageStart}" end="${pageStart + perPage - 1}">
            ...
            <article id="article${loop.index+1}">
                <h2 align="center">${games.key.getName()}</h2>
                <div class="line"></div>
                <ul class="events-list">
                    <c:forEach items="${games.value}" var="item" varStatus="innerLoop">
                        <li>
                            <form id="${games.key.getName()}Form${innerLoop.index+1}" method="POST"
                                ...
                                <a href="${item.getHref()}" class="event-photo"><img src=
